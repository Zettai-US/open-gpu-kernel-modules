...
Currently, the kernel modules can be built for x86_64, aarch64, or riscv64.
...
When building natively on riscv64, `TARGET_ARCH` is detected automatically and
the kernel-interface build maps it to the Linux kernel `ARCH=riscv` value.  If
the target kernel's `/lib/modules/$(uname -r)/build` symlink is unavailable,
pass the prepared kernel tree explicitly:

    make modules -j$(nproc) \
        SYSSRC=/path/to/prepared/linux/build

For Lanxin3 bring-up against kernel 6.18.3+, make sure the system has a native
C++ compiler and that `/lib/modules/$(uname -r)/build` points at the prepared
kernel tree:

    sudo apt-get install g++
    sudo ln -sfn /home/lanxin500/kernel-src/linux-6.18.3 \
        /lib/modules/$(uname -r)/build
    make modules -j$(nproc) NV_VERBOSE=0

If the modules build but insertion fails with `disagrees about version of
symbol`, the kernel tree's `Module.symvers` was not generated by the currently
booted kernel.  After an initial build has produced the `.ko` files, regenerate
the needed CRCs from the live kernel and rebuild:

    scripts/riscv-live-crc-overlay.sh \
        --kernel-src /home/lanxin500/kernel-src/linux-6.18.3 \
        --module-dir kernel-open \
        --apply
    make -C kernel-open clean_conftest
    make modules -j$(nproc) NV_VERBOSE=0

For non-interactive SSH sessions, pass `SUDO_PASSWORD` to the overlay script so
it can run the temporary live-CRC probe through `sudo`.

`IGNORE_MISSING_MODULE_SYMVERS=1` is only for smoke builds when the matching
kernel build tree does not provide a usable `Module.symvers`.  Production
modules should be built against the exact prepared kernel build output for the
running kernel.
